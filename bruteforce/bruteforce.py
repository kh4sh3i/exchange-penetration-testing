...
def parse_args():
    def check_arg_threads(value):
        try:
            ...
            raise Exception('Buffer size must be an integer')
        if value < 1 or value > 30:
            raise Exception('Buffer size must be an integer between 1 and 30')
        return value
    ...
    def check_arg_order(value):
        value = str(value)
        if value not in ['parallel', 'serie']:
    ...
    parser.add_argument('-w', '--workers', type=check_arg_threads, default=3, help='Number of threads (integer between 1 and 30)')
...
class HTTP_Auth():
    ...
    def check(url):
        ...
        # Target validation
        if target['error'] is None and target['authentication type'] in ['basic', 'digest', 'ntlm'] and target['status code'] == 401:
        ...
        while True:
            ...
            try:
                ...
                if test is None:
                    ...
                    if test['success']:
                        log.success('Authentication successful: Username: "{}" Password: "{}" URL: {}'.format(
                            test['username'], test['password'], test['target']['url']))
                    else:
                        if self.verbose:
                            log.debug('Authentication failed: Username: "{}" Password: "{}" URL: {}'.format(
                                test['username'], test['password'], test['target']['url']))
                    self.progress += 1
...
def main():
    ...
    if len(targets_included) > 0:
        ...
        if validation.lower() != 'y':
            log.info('Bruteforce not launched')
            exit(0)
